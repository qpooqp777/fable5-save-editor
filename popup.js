...
function renderInventory() {
  ...
  filtered.forEach(item => {
    ...
    card.innerHTML = `
      ...
      <div class="item-card-row select-row">
        <label style="font-size:10px;color:var(--text-muted)">屬性</label>
        <select class="inv-select inv-sel-ele" data-field="attr_ele" data-idx="${item._idx}">
          <option value="">無</option>
          <option value="fire" ${(item.attr||'').startsWith('fire')?'selected':''}>🔥 火</option>
          <option value="water" ${(item.attr||'').startsWith('water')?'selected':''}>💧 水</option>
          <option value="wind" ${(item.attr||'').startsWith('wind')?'selected':''}>🌪️ 風</option>
          <option value="earth" ${(item.attr||'').startsWith('earth')?'selected':''}>⛰️ 地</option>
          <option value="ice" ${(item.attr||'').startsWith('ice')?'selected':''}>❄️ 冰</option>
          <option value="light" ${(item.attr||'').startsWith('light')?'selected':''}>✨ 光</option>
          <option value="dark" ${(item.attr||'').startsWith('dark')?'selected':''}>🌑 暗</option>
        </select>
        <select class="inv-select inv-sel-lv" data-field="attr_lv" data-idx="${item._idx}">
          <option value="">-</option>
          <option value="1" ${(item.attr||'').endsWith('1')?'selected':''}>1</option>
          <option value="2" ${(item.attr||'').endsWith('2')?'selected':''}>2</option>
          <option value="3" ${(item.attr||'').endsWith('3')?'selected':''}>3</option>
          <option value="4" ${(item.attr||'').endsWith('4')?'selected':''}>4</option>
          <option value="5" ${(item.attr||'').endsWith('5')?'selected':''}>5</option>
        </select>
        <label style="font-size:10px;color:var(--text-muted);margin-left:8px">套裝</label>
        <select class="inv-select inv-sel-set" data-field="seteff" data-idx="${item._idx}">
          <option value="">無</option>
  ...
  c.querySelectorAll('select[data-field]').forEach(sel => {
    sel.addEventListener('change', () => {
      const idx=parseInt(sel.dataset.idx), field=sel.dataset.field, item=p.inv[idx];
      if (field==='attr_ele') {
        // 組合屬性：元素 + 等級
        const ele = sel.value;
        const lvSel = c.querySelector(`select[data-field="attr_lv"][data-idx="${idx}"]`);
        const lv = lvSel ? lvSel.value : '';
        item.attr = ele ? (ele + lv) : false;
      } else if (field==='attr_lv') {
        // 等級改變時，組合屬性
        const lv = sel.value;
        const eleSel = c.querySelector(`select[data-field="attr_ele"][data-idx="${idx}"]`);
...
function selectModalItem(id) {
  ...
  if(stats.req&&stats.req!=='all') {
    ...
    const reqNames=stats.req.split(',').map(c=>classMap[c]||c).join('、');
    statLines.push({l:' 職業限制',v:reqNames});
  }
...
function itemIconHTML(id) {
  ...
  const dir = TYPE_ICON_DIR[info.t] || 'items';
  const src = 'assets/icons/' + dir + '/' + encodeURIComponent(info.n) + '.png';
  return '<img src="' + src + '" alt="' + esc(info.n) + '" class="inv-item-icon" onerror="this.style.display=\'none\';this.nextElementSibling.style.display=\'flex\';"><span class="item-icon-fallback" style="display:none">&#128280;</span>';
}
